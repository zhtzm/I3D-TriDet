...
class FPNIdentity(nn.Module):
    ...
    def __init__(
        self,
        in_channels,      # 输入通道数,这是一个list
        out_channel,      # 输出通道数,一个常数
        scale_factor=2.0, # FPN层之间的下采样系数,因为特征list由backbone给出,因此这里要与backbone中系数相同
        start_level=0,    # 开始层
        end_level=-1,     # 结束层
        with_ln=True      # if to apply layer norm at the end
    ):
    ...
    def forward(self, inputs, fpn_masks):
        """
            FPNIdentity Network由LN层堆叠,并不影响尺寸大小
        """
        assert len(inputs) == len(self.in_channels)
        assert len(fpn_masks) ==  len(self.in_channels)
