...
def vedio2predict(video_dir, config_file, ckpt_file, batch_size=40, topk=10, level=0.25, mapping_file="data\\thumos\\mapping.json"):
    results = video_feature(video_dir, batch_size=batch_size)
...
def default_predict2show():
    ...
    t1 = time.time()
    results = vedio2predict(video_dir, config, ckpt, batch_size=256)
